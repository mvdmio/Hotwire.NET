...
public class AfterMultipleTurboActionTests
{
   [Fact]
   public async Task WithRequiredParameters()
   {
      ...
      var action = new AfterMultipleTurboAction(targetsSelector, new HtmlString(template));

      var result = await action.Render();

      result.Value.Should().Be(
          """
          <turbo-stream action="after" targets=".selector">
             <template>
                <test>template</test>
             </template>
          </turbo-stream>
