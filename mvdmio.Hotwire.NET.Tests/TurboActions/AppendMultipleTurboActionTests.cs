...
public class AppendMultipleTurboActionTests
{
   [Fact]
   public async Task WithRequiredParameters()
   {
      ...
      var action = new AppendMultipleTurboAction(targetsSelector, new HtmlString(template));

      var result = await action.Render();

      result.Value.Should().Be(
          """
          <turbo-stream action="append" targets=".selector">
             <template>
                <test>template</test>
             </template>
          </turbo-stream>
